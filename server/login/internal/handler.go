...
func handleMsg(msgID int /*m interface{}*/, h interface{}) {
	skeleton.RegisterChanRPC(msgID, h)
}

func init() {
	if !useMemoryDB {
		handleMsg(msg.MsgID_SignUp /*&msg.SignUp{}*/, handleSignUpDB)
		handleMsg(msg.MsgID_SignIn, handleSignInDB)
	} else {
		handleMsg(msg.MsgID_SignIn, handleSignInMem)
		handleMsg(msg.MsgID_SignUp, handleSignUpMem)
	}

	///保持用户的数据
	handleMsg(msg.MsgID_UpLoad, handleUploadDB)
	handleMsg(msg.MsgID_Register, handleRegister)

}

func handleRegister(args []interface{}) {
	log.Debug("handleRegister")
	log.Debug("args[0]:%v args[1]:%v", args[0], args[1])
	a := args[0].(gate.Agent)
	tmp := int32(msg.MsgID_Register)
	bytesBuffer := bytes.NewBuffer([]byte{})
	binary.Write(bytesBuffer, binary.LittleEndian, tmp /*msg.MsgID_Register*/)
	log.Debug("bytesBuffer.Bytes():%v", bytesBuffer.Bytes())
	a.WriteMsg(bytesBuffer.Bytes())
}
...
func handleSignInMem(args []interface{}) {
	...
	//	if Userdb[m.Name] == "" {
	//		///不存在用户
	//		a.WriteMsg(msg.Login_noexist /*&msg.State{msg.Login_noexist}*/)
	//	} else {
	//		if Userdb[m.Name] != m.Password {
	//			///用户名密码错误
	//			a.WriteMsg(msg.Login_mismatch /*&msg.State{msg.Login_mismatch}*/)
	//		} else {
	...
	//			UserAgent[m.Name] = a
	//			UserID++
	//			a.WriteMsg(msg.Login_success /*&msg.State{msg.Login_success}*/)
	//		}
	//	}
...
func handleSignUpMem(args []interface{}) {
	...
	//		a.WriteMsg(msg.SignUp_success /*&msg.State{msg.SignUp_success}*/)
	//	} else {
	//		a.WriteMsg(msg.SignUp_duplicate /*&msg.State{msg.SignUp_duplicate}*/)
	//	}
}
...
func handleSignInDB(args []interface{}) {
	...
	//			if a.UserData() != nil {
	//				a.WriteMsg(msg.Login_duplicate /*&msg.State{msg.Login_duplicate}*/)
