...
func (s *Skeleton) AsynCall(server *chanrpc.Server, msgID int /*id interface{}*/, args ...interface{}) {
	if s.AsynCallLen == 0 {
		panic("invalid AsynCallLen")
	}

	s.client.Attach(server)
	s.client.AsynCall(msgID, args...)
}

func (s *Skeleton) RegisterChanRPC(msgID int /*id interface{}*/, f interface{}) {
	if s.ChanRPCServer == nil {
		panic("invalid ChanRPCServer")
	}

	s.server.Register(msgID, f)
}

func (s *Skeleton) RegisterCommand(msgID int /*id interface{}*/, help string, f interface{}) {
	console.Register(msgID, help, f, s.commandServer)
}
