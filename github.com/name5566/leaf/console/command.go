...
func (c *ExternalCommand) msgID() int {
	return c._msgID
}
...
func (c *ExternalCommand) run(_args []string) string {
	...
	ret, err := c.server.Call1(c._msgID, args...)
	if err != nil {
		return err.Error()
	}
...
// goroutine not safe
func Register(msgID int, help string, f interface{}, server *chanrpc.Server) {
	for _, c := range commands {
		if c.msgID() == msgID {
			log.Fatal("command %v is already registered", msgID)
		}
	}

	server.Register(msgID, f)
...
func (c *CommandHelp) run([]string) string {
	output := "Commands:\r\n"
	for _, c := range commands {
		output += strconv.Itoa(c.msgID()) + " - " + c.help() + "\r\n"
	}
...
func (c *CommandCPUProf) msgID() int {
	return 10004
}
